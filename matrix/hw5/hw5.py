...
def morph(S, B):
    ...
    '''
    return [(b, exchange(S, list(), b)) for b in B]
...
row_space_2 = [list2vec([1,4,0,0]), list2vec([0,1,1,0]), list2vec([0,0,1,1])]
col_space_2 = [list2vec([1,0,0]), list2vec([0,2,0]), list2vec([0,0,1])]
...
def subset_basis(T):
    ...
    for v in T:
        returns.append(v)
        if is_superfluous(returns, len(returns)-1):
            returns.pop()
    return returns
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    for v in V_basis:
        UV_basis.append(v)
    UV_basis = coldict2mat(UV_basis)
    solution = solve(UV_basis, w)
    ulen = len(U_basis)
    vlen = len(V_basis)
    u_vec = sum(solution.f[i] * U_basis[i] for i in range(ulen))
    v_vec = sum(solution.f[ulen + i] * V_basis[i] for i in range(vlen))
    return (u_vec, v_vec)
...
def is_invertible(M):
    ...
    True
    '''
    return len(M.D[0]) is len(M.D[1]) and my_rank(list(mat2coldict(M).values())) is my_rank(list(mat2rowdict(M).values()))
...
def find_matrix_inverse(A):
    ...
    True
    '''
    ident_cols = mat2coldict(identity(A.D[0], one))
    return coldict2mat([solve(A,v) for v in ident_cols.values()])
...
def find_triangular_matrix_inverse(A):
    ...
    True
    '''
    ident_rows = mat2rowdict(identity(A.D[1], 1))
    labels = list(A.D[1])
    A_rows = mat2rowdict(A)
    print(A_rows)
    return rowdict2mat([triangular_solve(A_rows, labels, v) for v in ident_rows.values()])

...
def move2board(v):
    ...
    '''
    return Vec({'y1','y2','y3'}, {k:v[k]/v['y3'] for k in v.f.keys() if 'y3' in v.f.keys() and v.f['y3'] is not 0})
...
scaling = Vec(top_left[0].D, {('y1','x1'):1})
L = rowdict2mat([top_left[0],top_left[1],bottom_left[0],bottom_left[1],top_right[0],top_right[1],bottom_right[0],bottom_right[1],scaling])
b = Vec({0,1,2,3,4,5,6,7,8}, {8:1})
h = solve(L, b)
H = Mat(({'y1','y2','y3'},{'x1','x2','x3'}), {(y,x):val for (y,x),val in h.f.items()})
...
def mat_move2board(Y):
    ...
    '''
    return coldict2mat([move2board(v) for v in mat2coldict(Y).values()])

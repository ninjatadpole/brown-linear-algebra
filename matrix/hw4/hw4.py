...
indep_vec_1 = Vec({0,1,2,3}, {1,-1,0,0})
indep_vec_2 = Vec({0,1,2,3}, {0,1,-1,0})
indep_vec_3 = Vec({0,1,2,3}, {0,0,1,-1})
indep_vec_4 = Vec({0,1,2,3}, {-1,0,0,1})
...
def is_superfluous(L, i):
    ...
    if i is 0:
        newL = coldict2mat(L[i+1:])
    elif i is len(L)-1:
        newL = coldict2mat(L[:i])
    else:
        newL = coldict2mat(L[:i]+L[i+1:])
    result = solve(newL, L[i])
    residual = L[i] - newL * result
...
def is_independent(L):
    ...
    for i in range(len(L)):
        result = result and not is_superfluous(L, i)
    return result
...
def exchange(S, A, z):
    ...
    for i in range(len(full)):
        v = full[i]
        if is_superfluous(full, i) and v not in A and v is not z:
            result = v
            break
    return result

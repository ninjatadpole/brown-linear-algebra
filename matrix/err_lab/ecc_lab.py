...
def find_error(e):
    ...
    """
    return Vec({0, 1, 2, 3, 4, 5, 6},{x: one for x,v in mat2coldict(H).items() if e==v})
...
def find_error_matrix(S):
    ...
    """
    return coldict2mat({x:find_error(syn) for x,syn in mat2coldict(S).items()})
...
## Ungraded Task
CTILDE = C + noise(Mat((set(range(len(C.D[0]))),set(range(len(C.D[1])))),{}), 0.02)
